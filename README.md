...
- A React frontend with video upload, webcam capture, **audio pronunciation**, prediction history, and a dashboard
...
| **Audio / TTS** | espeak-ng (Arabic voice), auto-generated `.wav` files served statically |
...
└───────────────────┘       └────────┬──────────┘       └────────────────────┘
                          ...
                          ▼          ▼          ▼          ▼
                    ┌──────────┐ ┌────────┐ ┌────────┐ ┌──────────┐
                    │ Postgres │ │ MLflow │ │ Model  │ │ Audio    │
                    │ (5432)   │ │ (5000) │ │ .pt    │ │ data/    │
                    │ Users,   │ │ Experi-│ │ check- │ │ audio/   │
                    │ History  │ │ ments  │ │ point  │ │ espeak-ng│
                    └──────────┘ └────────┘ └────────┘ └──────────┘
```
...
│   │   └── generate_audio.py         #   Generates .wav pronunciation files via espeak-ng
│   │
...
│       │   ├── PredictionResults.jsx #   Prediction display with confidence bars + audio playback
...
│
├── data/
│   ├── raw/                          # Dataset files (not committed to git)
│   │   ├── KArSL/                    #   KArSL dataset (01/, 02/, 03/ signer folders)
│   │   └── labels/                   #   KARSL-502_Labels.xlsx
│   └── audio/                        # Auto-generated .wav pronunciation files (espeak-ng)
│       ├── 1.wav … 502.wav           #   One file per label, named by label ID
│
...
#### `src/utils/generate_audio.py`

Generates Arabic pronunciation audio files for every label using **espeak-ng** (open-source TTS with native Arabic support). Reads `label2text.json` and, for each label ID, calls `espeak-ng -v ar -s 130 -w data/audio/{id}.wav` via subprocess to produce a `.wav` file. Skips labels whose `.wav` already exists (idempotent). Called automatically on API startup so audio is always available without manual steps. Can also be run standalone: `python -m src.utils.generate_audio`.
...
FastAPI application entry point. On startup: (1) creates database tables, (2) generates any missing pronunciation `.wav` files via espeak-ng, and (3) loads the trained model checkpoint. Mounts `data/audio/` as a static file directory at `/audio` so the frontend can play pronunciation audio. Exposes `POST /predict/video` (accepts a video file upload) and `POST /predict/frames` (accepts base64-encoded frames from the webcam). Both endpoints decode the input, run inference via `ModelInference`, save the result to prediction history, and return top-K predictions with confidence scores. Also exposes `GET /health` for status checks.
...
- **Audio Pronunciation** — each prediction auto-plays its Arabic pronunciation (`.wav` from espeak-ng), with per-prediction play buttons and a mute toggle; audio is served from the API at `/audio/{label_id}.wav`
...
> **Note:** On the first startup, the API container automatically generates 502 `.wav` pronunciation files in `data/audio/` using espeak-ng (takes a few seconds). These files are cached on the volume-mounted `./data` directory and persist across restarts.
...
- **Audio Pronunciation** — predictions are spoken aloud automatically using pre-generated Arabic `.wav` files (espeak-ng); play button on each prediction, mute toggle for auto-play
...
| `GET` | `/audio/{label_id}.wav` | Static `.wav` pronunciation file for a label (auto-generated by espeak-ng) |
